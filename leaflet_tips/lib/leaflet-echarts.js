...
L.EchartsLayer = L.Class.extend({
    ...
    initialize: function(map, ec,oriData) {
        ...
        div.style.top = 0;
        div.style.left = 0;
        map.getPanes().shadowPane.appendChild(div);
        this._init(map, ec);
        this._data = oriData
    },
    ...
    _init: function(map, ec) {
        ...
        self.setOption = function(option, notMerge) {
            ...
            if(!self._data && option && option.series){
                self._data = []
                option.series.forEach(s=>{
                    self._data.push(s.data)
                })
            }
        ...
        function _zoomChangeHandler() {
            if(self._option.series){
                let a  = map.getZoom();
                if(task==null){
                    task=setTimeout(function(){self.setOption(self._option);task=null;},150);
                }else{
                    clearTimeout(task);
                    task=setTimeout(function(){self.setOption(self._option);task=null;},150);
                }
